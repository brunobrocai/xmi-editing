...
def tag_moralizations(tree, namespaces):
    ...
    for custom in custom_list:
        ...
        if moraliz and moraliz != 'Keine Moralisierung':
            ...
            tree = se.sofa_string_insert(tree, namespaces, insertion_begin, int(custom.get('begin')))
            tree = se.sofa_string_insert(tree, namespaces, insertion_end, int(custom.get('end')))
    return tree
...
def tag_subj_values(tree, namespaces):
    ...
    for custom in custom_list:
        ...
        if moraliz:
            insertion_begin = f'<MoralValue type="subjective expression" label="{moraliz}">'
...
if __name__ == "__main__":
    tree, root, namespaces = xh.parse_xmi('test.xmi')
